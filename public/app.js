...
// ============= GUIDE =============
function loadGuide(){
  document.getElementById('devref').innerHTML=Object.entries(DEV).map(([k,d])=>'<div class="rd"><div class="rdh" onclick="toggleRD(this)"><div class="rdi" style="background:'+d.bg+'"><span style="color:'+d.c+'">'+ico(d.ico,18)+'</span></div><span class="rdt">'+d.t+'</span><span class="badge" style="background:'+d.bg+';color:'+d.c+'">'+Object.values(d.ages).flat().length+' etapes</span><span class="rda">'+I.chevR+'</span></div><div class="rdb" data-domain="'+k+'"></div></div>').join('');
  document.getElementById('glist').innerHTML=Object.entries(GUIDE).map(([k,g])=>'<div class="gc" onclick="showGD(\''+k+'\')"><div class="gci" style="background:'+g.bg+'"><span style="color:'+g.c+'">'+ico(g.ico,20)+'</span></div><div style="flex:1"><div class="gct">'+g.t+'</div><div class="gcd">'+g.tips.length+' conseils bienveillants</div></div><span style="color:var(--tx4)">'+I.chevR+'</span></div>').join('')}

window.toggleRD=function(el){const body=el.nextElementSibling,arrow=el.querySelector('.rda'),dom=body.dataset.domain;const isOpen=body.classList.contains('op');body.classList.toggle('op');arrow.classList.toggle('op');if(isOpen)return;
  const d=DEV[dom];if(!d)return;const ages=Object.keys(d.ages);let cur=ages[0];const ak=devAgeKey(S.childAge);if(ages.includes(ak))cur=ak;
  ...
  showAgeContent(dom,cur)}

window.showAge=function(dom,age,btn){btn.parentElement.querySelectorAll('.agetab').forEach(t=>t.classList.remove('on'));btn.classList.add('on');showAgeContent(dom,age)}
...
async function loadDynamicContent(){
  try{
    ...
    if(dvr.ok){const dv=await dvr.json();if(dv.length>0){Object.keys(DEV).forEach(k=>delete DEV[k]);dv.forEach(dom=>{const ages={};(dom.steps||[]).forEach(s=>{if(!ages[s.ageGroup])ages[s.ageGroup]=[];let acts;try{acts=typeof s.activities==='string'?JSON.parse(s.activities):s.activities}catch(e){acts=[s.activities]}ages[s.ageGroup].push({l:s.label,acts:acts})});DEV[dom.key]={t:dom.title,ico:dom.icon,c:dom.color,bg:dom.bgColor,ages}})}}
